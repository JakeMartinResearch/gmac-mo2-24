...
**💿 slope_df_test**: We have imported the slopes extracted in LabChart during each phase of the experiment
...
slope_df_test <- bind_rows(slope_list) %>% 
  dplyr::mutate(resp_cat_date = paste0(respirometer_group, "_", start_date),
                chamber_n = str_extract(chamber_id, "\\d+"),
                id_prox = paste0(resp_cat_date, "_", chamber_n),
                time_hms = as_hms(time*3600),
...
```{r}
v3_mo2corr <- slope_df_2 %>% 
  dplyr::arrange(id, order) %>% 
  dplyr::select(id, order, o2, phase, mo2corr) %>% 
  dplyr::rename(mo2corr_v3 = mo2corr)

v2_mo2corr <- slope_df_test_2 %>% 
  dplyr::arrange(id, order) %>% 
  dplyr::select(id, order, o2, phase,  mo2corr) %>% 
  dplyr::rename(mo2corr_v2 = mo2corr)

v2_3_comp <- full_join(v3_mo2corr, v2_mo2corr, by = c("id", "order", "o2", "phase")) %>% 
  dplyr::mutate(v3_minus_v2 = mo2corr_v3-mo2corr_v2)
```

```{r}
v2_3_comp %>% 
  dplyr::filter(mo2corr_v2 < 0.01) %>% 
  ggplot(aes(x = mo2corr_v3, y = mo2corr_v2)) +
...
```{r}
v3_mo2corr_l <- slope_df_2 %>% 
  dplyr::arrange(id, order) %>% 
  dplyr::select(id, order, o2, phase, mo2corr) %>% 
  dplyr::mutate(version = "3")

v2_mo2corr_l <- slope_df_test_2 %>% 
  dplyr::arrange(id, order) %>% 
  dplyr::select(id, order, o2, phase,  mo2corr) %>% 
  dplyr::mutate(version = "2")

v2_3_comp_l <- rbind(v3_mo2corr_l, v2_mo2corr_l) 

v2_3_comp_l %>% 
  ggplot(aes(x = o2, y = mo2corr, colour = version)) +
...
```{r}
v2_3_comp_l %>% 
  dplyr::filter(id == "b_0_25nov_4") %>% 
  ggplot(aes(x = o2, y = mo2corr, colour = version)) +
  geom_point(alpha = 0.3) + 
  theme_clean() +
    labs(
      subtitle = "b_0_25nov_4",
      x = "o2",
      y = "mo2corr before transformation"
    )
```
...
if (!dir.exists(output_mods_bayes_wd)) {dir.create(output_mods_bayes_wd)}
```
...
```{r, echo=FALSE}
incremental_reg_bayes_wd <- file.path(output_fig_wd, "incremental_regressions")
...
if (!dir.exists(output_mods_bayes_global_wd)) {dir.create(output_mods_bayes_global_wd)}
```
...
```{r}
# setwd(output_mods_bayes_global_wd)
# 
# seed_number = 143019
# 
# # Set up parallel backend
# plan(multisession, workers = 4)  # Adjust workers based on system resources
# 
...
For the fish that did have Pcrits, proportionally slightly more fish were from the zero salinity treatment (20% *vs* 18%), fish were close to the mean weight (maybe slightly smaller). 
...
slope_tidy %>% 
  ...
  dplyr::reframe(n_pcrit_salinity_9 = sum(salinity_group == "9", na.rm = TRUE),
                 ...
                 rel_mean_weight = mean(mass, na.rm = TRUE)/mean_weight_all) %>% 
  gt()
```
