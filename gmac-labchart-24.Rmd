...
We will remove some trials which had errors. These are as follows: <br>
...
Here we remove the first 5 SMR cycles, positive raw slopes and MO2 calculated using less then 60 data points.
...
<br>
Now we remove the points after the chamber is opened (i.e. o2 increases in a closed phase)
...
Here we will estimate SMR using the mean of the lowest 3 values
...
Here we are transforming the MO2 values 

- MO2 = absolute value of the background and leak corrected mo2 slope from Labchart (mo2corr) times the net volume of the chamber (volume - fish mass), times 60, times 60, to achieve MO2 mg/h.
...
<br>
Here's the resulting data frame, we may want to filter these based on visual assessment of the models. 

```{r}
pcrit_conforming_list <- pcrit_model_df %>% 
  dplyr::filter(pcrit_nb_mo2_conforming > 0) %>% 
  pull(id)
```
...
for (id_i in ids) {
  ...
  # Add a title
  mtext(
    text = paste0(id_i, " (R2 = ", r2, "; p = ", P, "; Conforming points = ", conforming, ")"),
    side = 3, line = 2, adj = 0,
...
<br> 
Setting rules for Pcrit estimate <br>
- Three consecutive MO2 values below SMR (Frisk et al., 2012)
...
for (id_i in pcrit_list) {
  ...
  # Add a title
  mtext(
    text = paste0(id_i, " (R2 = ", r2, "; p = ", P, "; Conforming point = ", conforming, ")"),
    side = 3, line = 2, adj = 0,
    col = "blue", font = 2, cex = 1.2
  )
}
