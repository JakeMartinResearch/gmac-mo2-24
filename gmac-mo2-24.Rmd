---
title: The impact of isosmotic conditions on the metabolism and hypoxia tolerance
  of a purportedly oxyconforming teleost
author: Timothy D. Clark, Luis L. Kuchenmüller, Elizabeth C. Hoots, Maryane Gradito,
  Jake M. Martin
...
**SUMMARY**
This R code estimates the relationship between oxygen consumption (MO₂) and dissolved oxygen saturation (DO) in the Common Galaxias (*Galaxias maculatus*). It also estimates the critical oxygen saturation value for aerobic metabolism O<sub>2crit</sub>, commonly defined as the threshold below which the oxygen consumption rate can no longer be sustained. The associated article is titled *"The role of osmorespiratory compromise in metabolism and hypoxia tolerance of a purportedly oxyconforming teleost."* If you are reading the HTML version of this script, click the `Code` button in the top right to download the `.Rmd` file.

**AIM**
The article aims to test whether *Galaxias maculatus* can maintain oxygen consumption (MO₂) as ambient DO declines and, if so, at what level it reaches the critical oxygen saturation value for aerobic metabolism (O<sub>2crit</sub>).
...
Clark, T. D., Kuchenmüller, L. L., Hoots, E. C., Gradito, M., & Martin, J. M. (2025, May 28). The role of osmorespiratory compromise in hypoxia tolerance of the purportedly oxyconforming teleost Galaxias maculatus. https://doi.org/10.17605/OSF.IO/GFXCA

<!------------------------------->

# 🔧 Set up: Packages and knit settings
...
pkgs <- c(
  # ----- Data Visualisation -----
  "ggthemes", "bayesplot", "gt", "gtsummary", "plotly", "qqplotr", "gridExtra", "flextable",
...
```{r}
wd <- getwd()
input_data_wd <- file.path(wd, "input-data") # creates a variable with the name of the wd we want to use
```

**📥 mod_data_wd**: Directory for model output data estimated slopes

```{r}
mod_data_wd <- file.path(wd, "mod-data")
```
...
<span style="color:#f692B4">raw MO₂ calculations</span> are shown in pink, 
<span style="color:#6E7889">MO₂ calculations after correcting for background respiration</span> are shown in grey, and 
<span style="color:#53af8b">MO₂ calculations after correcting for background respiration and oxygen diffusion dynamics</span> are shown in green.
...
**Figure S1**: Individual fish-level plots showing the relationship between dissolved oxygen (DO) percentage and metabolic oxygen consumption rate (MO₂, in mg O₂/g/h) for each 20-min interval. Each panel displays data for a single fish (fish ID in the top left of the plot). Points are colour-coded by the type of MO₂ calculation:
<span class="pink">raw MO₂</span> (pink, #f692B4),
<span class="grey">background-corrected MO₂</span> (grey, #6E7889), and
<span class="green">fully corrected MO₂ (accounting for background respiration and oxygen diffusion dynamics)</span> (green, #53af8b).
...
***Figure S2:*** interactive plot of metabolic rate measurements (MO₂; mg O₂ g^-1^h^-1^) by dissolved oxygen percentage (DO) for all fish, including all estimates during the SMR phase (i.e. intermittent phase). Individual linear regression were fitted for visual reference, and do not represent the best fitting regression.
...
These plots show Metabolic rate measurements (MO₂; mg O₂ g^-1^h^-1^ by O<sub>2</sub>) at each of the four experimental phase (the over night SMR intermittent-flow respirometry phase, closed phases at 50%, 75% or 100% O<sub>2</sub>). The estimated SMR value is represented as a dashed red line.
...
slope_tidy_smr %>%
  ...
  dplyr::reframe(temp_range = paste0(round(min(temp, na.rm = TRUE),2), "–", round(max(temp, na.rm = TRUE),2)),
            mass_range = paste0(round(min(mass, na.rm = TRUE),2), "–", round(max(mass, na.rm = TRUE),2)),
          cycle_range = paste0(round(min(cycle, na.rm = TRUE),2), "–", round(max(cycle, na.rm = TRUE),2)),
...
⏭️ **Skip this step** if you have downloaded the saved model `mo2_mod_gamma.rds` from GitHub (or pulled the entire GitHub project)
...
mass<sub>2</sub> ≈ mass<sub>1</sub> x 2.718
...
```{r}
smr %>%
  dplyr::reframe(temp_range = paste0(round(min(temp_mean, na.rm = TRUE),2), "–", round(max(temp_mean, na.rm = TRUE),2)),
            mass_range = paste0(round(min(mass, na.rm = TRUE),2), "–", round(max(mass, na.rm = TRUE),2)),
          cycle_range = paste0(round(min(cycles, na.rm = TRUE),2), "–", round(max(cycles, na.rm = TRUE),2)),
          number_fish = length(unique(id))  
          ) %>% 
  gt()
```
...
Checking if this pattern is driven by mass, as request by the reviewer. 


```{r}
mass_polynomial <- best_mod %>% 
  dplyr::group_by(model_type) %>% 
  dplyr::select(id, model_type) %>% 
  left_join(smr, ., by = "id")


mass_polynomial_tbl <-  mass_polynomial %>% 
  dplyr::group_by(model_type) %>% 
  dplyr::reframe(mean_mass = round(mean(mass, na.rm = T),2),
                 sd_mass = round(sd(mass, na.rm = T),2),
                 n = length(id)) %>% 
  ...
  cols_label(
    model_type = "Best polynomial fit",
...
```{r}
priors_default <- get_prior(mass_bf, data = mass_polynomial, family = gaussian())
priors_default %>% 
  gt()
```
...
⏭️ **Skip this step** if you have downloaded the saved model 'mass_mod.rds'

```{r, echo=FALSE}
# mass_mod <- brm(mass_bf,
#                data = mass_polynomial,
#                cores = 4,
#                chains = 4,
#                prior = priors_default,
#                warmup = 1000,
...
#                save_pars = save_pars(all=TRUE),
#                sample_prior = TRUE,
#                file = paste0(output_mods_wd, './mass_mod'))
# print("Model complete")
```
...
```{r}
mass_mod <-  readRDS(file = paste0(output_mods_wd, './mass_mod.rds'))
```
...
```{r}
color_scheme_set("red")
p <- pp_check(mass_mod, type = "dens_overlay")
p
```
...
```{r}
model_est <- fixef(mass_mod, probs = c(0.025, 0.975)) %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column(var = "Predictor") %>% 
  dplyr::mutate('β' = round(Estimate, 3),
                Q2.5 = round(Q2.5, 3),
...
```{r}
em_results <- emmeans(mass_mod, ~ model_type) 
...
**Table S3:** Posterior mean differences in mass (g) (estimate) and 95% highest posterior density intervals (95% CrI) are shown for all pairwise comparisons among model types (0th- to 3rd-order polynomial). Estimates are derived from a Gaussian Bayesian model and represent differences in marginal posterior means on the response scale. A meaningful difference was inferred when the 95% CrI did not overlap zero.

```{r}
tableS3 <- contrast(em_results, method = "pairwise") %>% 
  as.data.frame() %>%
  mutate(across(where(is.numeric), ~ round(.x, 2))) %>% 
  mutate(
    contrast = contrast %>% 
      str_replace_all("lm_0", "0th-order polynomial") %>%
      str_replace_all("lm_1", "1st-order polynomial") %>%
      str_replace_all("lm_2", "2nd-order polynomial") %>%
      str_replace_all("lm_3", "3rd-order polynomial") %>%
      str_replace_all(" - ", " \u2212 "),   # nicer minus sign
    cri = paste0(lower.HPD, " to ", upper.HPD)
  ) %>% 
  select(contrast, estimate, cri) %>% 
  gt() %>% 
...
### O₂crit numerical rules

We applied a numerical rule to asses if a O<sub>2crit</sub> was observed. Specifically, we filtered for only cases were at the lowest O₂ value three consecutive MO₂ measures full below our SMR and fifth percentile of the MO2 values observed at dissolved O2 levels > 80%. In the model output these are called nb_mo2_conforming points.
...
We conducted visual inspection of all 58 fish to determine if O<sub>2crits</sub> were present. We have loaded this as a data frame **💿 o2crit_check**
...
**Figure S9**: Oxygen consumption rate (ṀO₂; mg⁻¹ O₂ h⁻¹) and ambient dissolved oxygen (DO; % saturation) plotted for each fish that passed both past visual and  numerical assessment for O<sub>2crit</sub>. The horizontal dashed lines show SMR, the solid sloped lines represent the linear best fit for ṀO values below regulatory failure. The vertical soild lines represent O<sub>2crit</sub> (the intersection of the linear best fit for ṀO values below regulatory failure and SMR).
...
fig3 <- time_bin_df %>% 
  ...
  y = expression("MO"[2]~"(μmol O"[2]~"g"^-1~"h"^-1)) +
