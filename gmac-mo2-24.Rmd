...
**Figure S4:** Plot of all MO<sub>2</sub> measures during SMR phase by salinity treatment. The small points are the raw observed values, the shaded area behind the points is the a kernel density of the observed data, the box plot shows the median and interquartile range (IQR), and the large point shows the mean.
...
We also used a numerical rule to asses if a O<sub>2crit</sub> was observed.
...
Fig_4 <- ggplot() +
  ...
  geom_line(data = smooth_segments,
            aes(x = DO, y = MO2_pred, colour = id), size = 1.2) +
...
Here we are showing the out put of the Chabot O<sub>2crit</sub> model, 
