...
This R code is used to estimate mo2 and Pcrit (commonly understood as the threshold below which oxygen consumption rate can no longer be sustained) in the Common galaxias (Galaxias maculatus). The assocated article is "The role of osmorespiratory compromise in hypoxia tolerance of the purportedly oxyconforming teleost *Galaxias maculatus*"
...
Here are some of the custom function used in the script. They are authored by Chabot D.
...
Making the calcO2crit function 
...
Directory for Labchart estimated slopes
...
This code creates a new folder for data output called tidy-output-data, we then save this wd as the output_wd

```{r}
output_data_wd <- paste0(wd, "./output-data")
ifelse(!dir.exists("output-data"), dir.create("output-data"), "Folder already exists")
```

This is where we will put the figures
...
These are the slopes mainly extracted in LabChart during each phase of the experiment
...
This is the meta data for each chamber

We are also adding volume based on chamber type.
...
*NOTES"*: 
(1) There's something wired going on with *a_0_25nov_2* it seems like many of the raw MO2 values are positive.
(2) Often there seems to be a low MO2 vaule at about 5 mg/L O2
<br>



```{r}
output_fig_slopes_wd <- paste0(output_fig_wd, "./slopes")
ifelse(!dir.exists(output_fig_slopes_wd), dir.create(output_fig_slopes_wd), "Folder already exists")
...
for (id_i in ids) {
  ...
  smr_l3 <- labchart_tidy_fish %>% 
    ...
    plot <- labchart_tidy_fish %>% 
      dplyr::filter(id == id_i) %>% 
      ggplot(aes(y = MO2, x = o2)) + # Group by 1 to ensure a single line
      ...
      theme_clean() +
      labs(subtitle = paste0(id_i, " Pcrit"),
           x = "Mean o2 (mg_per_l)",
           y = "abs(mo2) (mg_per_l)")
    plot
    
    filename = paste0(output_fig_slopes_wd, "./slopes_", id_i, ".pdf")
...
```{r}
output_fig_pcrit_chabot_wd <- paste0(output_fig_wd, "./model_chabot")
ifelse(!dir.exists(output_fig_pcrit_chabot_wd), dir.create(output_fig_pcrit_chabot_wd), "Folder already exists")
...
for (id_i in ids) {
  ...
  r2 <- pcrit_model_df %>% dplyr::filter(id == id_i) %>% 
    dplyr::mutate(pcrit_r2 = round(pcrit_r2, 3)) %>% 
    pull(pcrit_r2)
  
  pdf(file = paste0(output_fig_pcrit_chabot_wd, "./", "pcrit_chabot_", id_i, ".pdf"), 
        width = 8, height = 6)
...
```{r}
output_fig_pcrit_100c_wd <- paste0(output_fig_wd, "./pcrit-100c")
ifelse(!dir.exists(output_fig_pcrit_100c_wd), dir.create(output_fig_pcrit_100c_wd), "Folder already exists")
...
for (id_i in ids) {
  ...
  tryCatch({
    # Open a PDF device
    pdf(file = paste0(output_fig_pcrit_100c_wd, "./", "pcrit_plot_", id_name, ".pdf"), 
        width = 8, height = 6)
